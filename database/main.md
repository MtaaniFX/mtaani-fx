**Part 1: Database Tables and initial policies**

**1. Roles Table**

This table will define the roles within our application and their hierarchical relationship.
...
-- Insert default roles with hierarchy (admin > staff > user)
INSERT INTO roles (name, parent_role) VALUES
    ('admin', NULL),  -- top-level role
    ('staff', (SELECT id FROM roles WHERE name = 'admin')), -- staff inherits from admin
    ('user', (SELECT id FROM roles WHERE name = 'staff')); -- user inherits from staff
```
