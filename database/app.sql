...
INSERT INTO individual_account_subtypes (name)
VALUES ('Normal'),
       ('Locked');
...
(
    ...
    account_type_id INTEGER REFERENCES account_types (id) DEFAULT (SELECT id
                                                                   FROM account_types
                                                                   WHERE name = 'Group'),
...
INSERT INTO group_roles (name, parent_role)
VALUES ('owner', NULL), -- top-level role within a group
       ...
       ('member', (SELECT id FROM group_roles WHERE name = 'secretary'));
-- regular member
...
CREATE OR REPLACE FUNCTION check_group_user_role(p_user_id UUID, p_group_id UUID, role_name TEXT)
    RETURNS BOOLEAN AS
$$
DECLARE
    role_exists BOOLEAN;
